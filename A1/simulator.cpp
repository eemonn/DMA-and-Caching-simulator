...
void insert_data( string line )
{
  ...
  for ( i=0 ; i<line.length() ; i+=4 )
  {
    ...
    //reshaping data memory according to cache-(block size 2 (2 words or 4 bytes), number of blocks 4)
     //  DATA_SIZE=DATA_SIZE/BLOCK_SIZE;
      // WORD_SIZE=2*BLOCK_SIZE;
     /*  unsigned char byte3,byte4;
       ...
       for(int i=0; i<line.length();i+=4){
           ...
           if ( data_index < DATA_SIZE*WORD_SIZE )
          {
            sscanf( ascii_data, "%02hhx%02hhx%02hhx%02hhx", &byte1, &byte2, &byte3, &byte4 );
            data[data_index][0] = byte1;
            data[data_index][1] = byte2;
              data[data_index][2]=byte3;
              data[data_index][3]=byte4;
              data_index++;
          }
       }*/
}
