...
//word transfer
#define TOTAL_WORD    4
#define WORD_TRANSFER (TOTAL_WORD)/(BLOCK_SIZE) // calculating how much the loop will execute. for example blocksize 1 means data memory will have 1 word each block so need to execute at most 4 times for burst transfer. 4 words or less at a time then loop terminates.
...
// we need to store cache data and the current state of the cache entries- write through update policy
struct CACHE_ENTRY
{
    bool           valid;
...
// three registers for manipulating hard drive
struct HDD_REGISTERS{
    unsigned short status_reg; //satus register
    unsigned short hdd_reg; // hard drive address register
    unsigned short mem_reg; // memory address register
};

struct HDD_REGISTERS hdReg;
...
Phase execute_instr();
Phase write_back();
Phase handling_interrupts(); //phase for dma transfer

////////////////////////////////////////////////////////////////////
// local variables

//keep track of the word transfer of a cycle(dma)
static bool isMore=false;   //find out if there is still any word left after burst transfer(4 words at a time)
static bool track=false;    // tracker for pooling
static unsigned short numWords=0;   //words that needs to be transfer
static unsigned short lsb=2;    //finding lsb (1 for write and 0 for read),that's why setting different number
static unsigned short tag=0;    //tag for memor address

//hard drive which is 4096 words in size
static unsigned char hdd[DISK_SIZE][WORD_SIZE];

//max number of blocks to transfer words
...
{
    fetch_instr, decode_instr, calculate_ea, fetch_operands, execute_instr, write_back,handling_interrupts
};
...
//this time using wirite through cache policy so no need of any dirty bits
...
//using write through cache update policy. writing memory and cahe at the same time
Phase cache_write()
{
    ...
    if ( state.MAR < (DATA_SIZE*BLOCK_SIZE*2) )
    {
        ...
        //write the word to main memory at the same time when write to cache- write thorugh
...
Phase write_back()
{
    Phase rc =  HANDLING_INTERRUPT;//FETCH_INSTR;
    ...
    switch( opcode() )
    {
        ...
        case MOVE_OPCODE:
            ...
            if ( mode() & 0x04 ){
                ...
                //intercepting before going to memory adderss 0,1,2. seeting up the hdd registers when memory address is 0,1,2 or go to cache
                if(registers[reg]==0){
                    hdReg.status_reg=state.MDR;
                    track=true; // tracks whenever enters memory address 0
                }
                else if(registers[reg]==1){
                    hdReg.hdd_reg=state.MDR;
                }
                else if(registers[reg]==2){
                    hdReg.mem_reg=state.MDR;
                }
                else
                    rc = cache_write();
            }
...
//DMA transfer phase( transfer 4 word at a time in a cycle)
Phase handling_interrupts(){
    Phase rc=FETCH_INSTR;
    unsigned short offset;
    int block_id; //block id for cache invalidate
    
    if(isMore) // in polling
        numWords= hdReg.status_reg;
    else{ //before polling
        lsb= hdReg.status_reg & 1; //finding whether it is read(0) or write(1) operation.
        numWords= hdReg.status_reg >> 1; // how many words are here.
        offset = addr2offset( hdReg.mem_reg ); // calculating offset for memory
        tag= addr2tag( hdReg.mem_reg ); //calculating block number from memory
    }
    
    
    if(numWords>0 && track){// initiate dma transfer as there are words to transfer.
        
        if(lsb==1){//write to hdd from main memory
            //transfering 4 words or less than 4 words at a time-burst transfer then exit from the loop and the phase. if any words left come back to this later
            for(int i=0;i<howMany && numWords!=0 ;i++){ // transfer 4 or less words at a time.
                for(int j=0;j<BLOCK_SIZE && numWords!=0 ;j++){
                    hdd[hdReg.hdd_reg][0]=data[tag][j][0]; //transfering words
                    hdd[hdReg.hdd_reg][1]=data[tag][j][1];
                    numWords--; // decreasing number of words- left to transfer
                    hdReg.hdd_reg++;//updating hd address register
                }//inner for
                
                tag=tag+1;;//updating block number for next transfer
            }//outer for
        }//lsb=1 if
        else{// lsb=0 means read from hdd and write to memory (need to do invalidate and bus snoope)
            
            //transfering 4 words or less than 4 words at a time-burst transfer then exit from the loop and the phase. if any words left come back to this later
            
            for(int i=0;i<howMany && numWords!=0 ;i++){// transfer 4 or less words at a time.
                for(int j=0;j<BLOCK_SIZE && numWords!=0 ;j++){
                    data[tag][j][0]=hdd[hdReg.hdd_reg][0]; //transfering words
                    data[tag][j][1]=hdd[hdReg.hdd_reg][1];
                    numWords--; // decreasing no. of words after every transfer
                    hdReg.hdd_reg++; // updating hdd reg for next transfer
                    
                    //cache invalidating when found that tag in cache
                    if(find_block(tag,block_id))
                        dictionary[block_id].valid=false;
                    
                }//inner for
                tag=+1; // updating tag for next transfer
            }//outer for
        }//else
    }//first if (numWords>0 && track)
    
    track=false; // it will true when entering memory 0 next time.
    
    if(numWords>0)
        isMore=true; // there is still words left for polling
    else{//no more words to transfer
        isMore=false;
        hdReg.status_reg=0; // making status reg 0
    }
...
//writing into file-hdd contents
void file_write(){
    FILE *fp;
    fp=fopen("hddcontents", "w");
    fwrite(hdd,1,sizeof(hdd),fp);
}
...
void initialize_system()
{
    ...
    //initialize the hdd contents to be empty
    ...
    //calculating blocks  for max words transfer. for example WORD_TRANSFER=4 so it will take 4 blocks to transfer 4 words at a time, since each block contain 1 words here.
    if(WORD_TRANSFER==0)
        howMany=1;
    else
        howMany=WORD_TRANSFER;
}
...
int main (int argc, const char * argv[])
{
    ...
    if ( load_files( argv[1], argv[2] ) )
    {
        ...
        // writing to file hard drive contents
        file_write();
